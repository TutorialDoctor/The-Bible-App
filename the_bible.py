...
def selectionToThoughts(sender):
	...
	txt = contents.text
	if txt[beg:end] != '':
		thought_bubble.text = thought_bubble.text +heading.text+'\n'+txt[beg:end]+'\n\n'
	else:
		thought_bubble.text = thought_bubble.text +heading.text+'\n'+txt+'\n\n'
...
def freeze(sender):
	if sender.value == True:
		thought_bubble.editable=True
	if sender.value==False:
		thought_bubble.editable=False
...
def load_thoughts(sender):
	with open('Notes/'+thoughts_file,'r',encoding='utf-8') as infile:
		...
		if selection==1:
			thought_bubble.text = infile.read()
		elif selection==2:
			None
...
def forward_to_thoughts(sender):
	thought_bubble.text = thought_bubble.text +'\n\n'+search_selection.text
	sound.play_effect('rpg:BookFlip2')
...
def change_font_size(sender):
	...
	thought_bubble.font = ('TimesNewRomanPSMT',19+font_slider.value*5)
...
def show_thoughts(sender):
	thoughts_view.hidden=False
...
def create_menu():
	...
	menu_items = [ui.ButtonItem(title=keys[int(x)],image=ui.Image(values[int(x)])) for x in images]
	bible.right_button_items = menu_items
...
def add_search():
	search = ui.TextField()
	search.height=32
	search.width=bible.width/5
	items = bible.right_button_items
...
testaments = top_pane['testaments']
testaments.action=test
#testaments2 = left_pane['testaments2']
thought_bubble = bible['thoughts']['thought_bubble']
thoughts_view= bible['thoughts']
thoughts_view.hidden=True
thoughts_switch = thoughts_view['switch']
thoughts_switch.action = freeze
thoughts_button = top_pane['btn_thoughts']
thoughts_button.action = selectionToThoughts
thoughts_save_button = thoughts_view['btn_save_thoughts']
thoughts_save_button.action = save_thoughts
hide_thoughts_button = thoughts_view['hide_thoughts_button']
hide_thoughts_button.action = hide_thoughts
show_thoughts_button = top_pane['btn_show_thoughts']
show_thoughts_button.action = show_thoughts
thoughts_title = thoughts_view['thoughts_title']
view_thoughts_button = top_pane['btn_view_thoughts']
view_thoughts_button.action = view_thoughts
load_button = top_pane['btn_load']
load_button.action = load_thoughts
clear_button = thoughts_view['btn_clear']
...
save_bookmarks_button=top_pane['btn_save_bookmarks']
...
settings_button = bible.left_button_items[2]
